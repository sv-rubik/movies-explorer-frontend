...
function MoviesCardList({movies, isLoading, onLike, onDelete, savedMovies, isMoviesFound, serverError}) {
  ...
  const getVisibleMoviesCount = useCallback(() => {
    if (isDesktop) {
      return sixteenCardsToShow; // При широком экране отображаем 16 шт
    } else if (isMiddleSize) {
      return twelveCardsToShow; // При экране вне макета диплома отображаем 12 шт
    } else if (isTablet) {
      return eightCardsToShow; // При среднем экране отображаем 8 шт
    } else {
      return fiveCardsToShow; // При узком экране отображаем 5 шт
    }
  ...
  const handleShowMore = () => {
    const showAnotherMoviesRow = () => {
      if (isDesktop) {
        return fourCardsToAdd; // При широком экране добавляем 4 карточки
      } else if (isMiddleSize) {
        return threeCardsToAdd; // При экране вне макета диплома добавляем 3 карточки
      } else {
        return twoCardsToAdd; // При узком экране добавляем 2 карточки
      }
    };
